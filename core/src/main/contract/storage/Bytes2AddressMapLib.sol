...
library Bytes2AddressMapLib
{
    ...
    event LogHistory(address txOrigin, address msgSender, bytes cmd, bytes key, address value);


    function add(Map storage self, bytes key, address value) internal returns (bool ret)
    {
    ...
    function set(Map storage self, bytes key, address value) internal returns (bool ret)
    {
        uint keyIndex = self.data[key].keyIndex;
        if (keyIndex > 0) {
    ...
    function insert(Map storage self, bytes key, address value) internal returns (bool replaced)
    {
        uint keyIndex = self.data[key].keyIndex;
        if (keyIndex > 0) {
            emit LogHistory(tx.origin, msg.sender, COMMAND_MODIFY, key, self.data[key].value);
    ...
    function get(Map storage self, bytes key) internal view returns (address)
    {
    ...
    function iterate_get(Map storage self, uint keyIndex) internal view returns (bytes key, address value)
    {
    ...
    function insertBatch(Map storage self, bytes[] keyList, address[] valList) internal returns (uint size)
    {
    ...
    function getBatch(Map storage self, bytes[] keys) internal view returns (address[] memory valList)
    {
    ...
    function getByIndex(Map storage self, uint start, uint num) internal view returns (bytes[] memory keyList, address[] memory valList, uint retNum, uint nextIndex)
    {
    ...
    function getAll(Map storage self) internal view returns (bytes[] memory keyList, address[] memory valList, uint retNum)
    {
        uint nextIndex = 0;
